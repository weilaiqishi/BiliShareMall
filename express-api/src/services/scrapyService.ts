...
// ScrapyItem 数据库操作
export const createScrapyItem = (params: {
  Name: string
  Cookie: string
  searchParams: SearchCategoryRequestBody
}) => {
...
export const updateScrapyItem = (item: ScrapyItem) => {
  const stmt = db.prepare(`
        ...
        WHERE ID = ?
    `)
  const info = stmt.run(item.NextToken, item.Status, Date.now(), item.ID)
  return info.changes
}
...
export const scrapyLoop = async (taskId: number, cookie: string) => {
  ...
  try {
    ...
    while (!signal.aborted) {
      console.log(
        `Running scrapy task -> ID, NextToken -> `,
        item.ID,
        item.NextToken,
      )
...
export const scrapyTask = async (
  ...
  try {
    ...
    const response = await axios.post(
      `${baseUrl}/mall/noah/search/category/v2`,
      requestBody,
      {
        headers: {
    ...
    console.log(
      `Scrapy task ${item.ID} -> item.NextToken, GoodsList.length, response.data?.data -> `,
      item.NextToken,
      GoodsList.length,
      omit(response.data?.data, ['list']),
    )
    if (GoodsList.length > 0) {
      try {
        ...
        console.error(
          `Scrapy task ${item.ID} failed with error: -> insertSearchGoodsItems or updateScrapyItem -> `,
          err,
        )
        return false
      }
      ...
      console.error(
        `Scrapy task ${item.ID} failed with code ${data.code}: ${data.message}`,
      )
      return false
    }
  } catch (error) {
...
export const startTask = async (taskId: number, cookie: string) => {
  if (currentRunningTaskId !== null) {
    console.log(
      `Task ${currentRunningTaskId} is already running. Cannot start new task. -> taskId -> `,
      taskId,
    )
    return
  }
...
export const stopTask = (taskId: number) => {
  ...
  if (abortController) {
    ...
    console.log(
      `Scrapy task is not running or already stopped. -> taskId -> `,
      taskId,
    )
  }
}
