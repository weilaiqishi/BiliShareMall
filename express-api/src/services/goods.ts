...
export function searchGoodsItems(params: SearchGoodsItemsParams): PaginatedResult<SearchCategoryGoodsItem> {
    ...
    const result = db.prepare(countQuery).get(...countQueryParams) as { 'COUNT(*)': number };
    const total = result['COUNT(*)'];
    const data = db.prepare(query).all(...queryParams) as SearchCategoryGoodsItem[];
